...
class ListComponent(object):
    ...
    def Equals(self, other):
        ...
        """
        if (self.__mType == other.GetType() and
           self.__mValue == other.GetValue()):
            return True
        return False

    def EqualsWithConnection(self, other):
        """Funcion igual, compara tipo y valor de 2 list conmponentes
        y en el caso de ser iguales ademas compara las conexiones
        ...
        """
        if (self.__mType != other.GetType() or
           self.__mValue != other.GetValue()):
            return False
    ...
    def __lt__(self, other):
        """ Sobrecarga del operador menor que, donde se comparan
        dos list components y se comprueba que alguno de los
        elementos es menor

        Devuelve verdadero si alguno de los elementos de
        listComponent es menor, falso en otro caso
    ...
    def CanTurn(self):
        """ Metodo que indica si un componente puede ser girado o no.
        Elementos que se pueden girar: R, L, C, SHORTCUT,
        XSWITCHOPEN, XSWITCHCLOSE, IPROBE
    ...
    def Dump(self):
        """ Metodo que crea y devuelve una cadena con los elementos
        del componente

        Devuelve un string con el contenido de todos los elementos
        del componente
        """
        out = "Component: "
    ...
    def AddConnection(self, con):
        """
        Anade una conexion al componente
        """
        self.__mConnections.append(con)

    def SetValue(self, value):
        """
        Anade el valor al componente
        """
        self.__mValue = value

    def SetType(self, newtype):
        """
        Anade el tipo al componente
    ...
    def GetSpecialToken(self, name):
        ...
        Len = 0
        # si no ha encontrado un espacio en blanco es porque
        # la linea ya se ha acabado entonces el tamano a copiar
        # en el string nuevo es tamano total
        # - donde empieza - el tamano del parametro:
        # ejemplo:( VDC+6V_3	D	max:6	imax:0.5 ),
        # name=MAX: mSpecial=max:6 imax:0.5. Start:4 end=6 Len=1
        if end == -1:
