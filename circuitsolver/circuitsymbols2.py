...
# este debe ir en el main quitar de aqui una vez chequeado el modulo
DEBUG_OUT = False
...
class SymbolTable (object):
    ...
    def __init__(self):
        ...
        # Contiene el nombre de los simbolos, en visir original contiene
        # estructuras name con tamaño max 16
        self.__sSymbolLookup = []
    ...
    def Ref(self, sym1, sym2):
        ...
        Obtiene ambos simbolos, si ambos existen, los mezcla y
        actualiza el indice.
        Parametros:
        sym1 -- Simbolo 1
        sym2 -- Simbolo 2'''
        ...
        if idx1 != -1 and idx1 == idx2:  # Son iguales?
            return True
        # son los dos distintos de -1? Es decir existian en el vector los dos
        if idx1 >= 0 and idx2 >= 0:
            if idx1 < idx2:
                if self.MergeNodes(idx1, idx2, sym2) is False:
                    return False
            else:
                if self.MergeNodes(idx2, idx1, sym1) is False:
                    return False
    ...
    def Insert(self, sym, node):
        ...
        definido en otro lugar ya que la función se utiliza
        con o sin verificación'''
        # finds the symbol and adds the node to it
        # if the node exists elsewhere, it will return false
        # Notice: Can't just return false if the node is defined
        # elsewhere as the function is used with or without verification
    ...
    # eliminado out ya que solo es para el retur
    def ToSymbolName(self, symbol):
    ...
    def LookupSymbol(self, symbol):
        '''Tenemos el nombre de un simbolo y recorremos vector
         self.__sSymbolLookup para ver si ya se ha almacenado
         el nombre y obtener asi su indice de no existir retornamos -1.
         Parametro:
         symbol -- nombre a buscar'''
        for i in range(
                0, len(self.__sSymbolLookup)):  # Recorremos todo el vector
    ...
    # equivale a cambiar el valor de la variable que retorna, como no lo
    # podemos hacer en python pasamos dos parametros en ese caso
    def GetSymbolRef(self, *arg):
        ''' Retorna el valor del simbolo
        Parametros:
        *arg -- argumento de longitud variable, tamaño uno es el simbolo
        a a buscar tamaño dos es el simbolo a buscar y el indice
        por el cual sustituir
        '''
        symbol = arg[0]
        if len(arg) == 2:
            newvalue = arg[1]
        ...
        if len(self.__mSymbolToNodeIdx) <= newidx:
            # python no tiene resize asi que lo hacemos nosotros,
            # añadimos None
            while len(
                    self.__mSymbolToNodeIdx) <= (
                    newidx + 1) * 2:
    ...
    def DumpSymbols(self):
        '''Retorna  la cadena con los nombres de los simbolos y
        su correspondiente indice'''
        outbuffer = ""
        for i in range(0, len(self.__sSymbolLookup)):
            outbuffer = outbuffer + \
                "(" + str(self.__sSymbolLookup[i]) + "-" + \
                str(self.__mSymbolToNodeIdx[i]) + ") "
        return outbuffer
    ...
    def MergeNodes(self, dst, src, srcsym):
        '''Cambia el indice del nodo desde src a dst, es decir,
        busca el indice dentro del vector de simbolos que coincide
        con src y lo cambia por dst, tambien lo cambia en
    ...
    def CreateNode(self, *arg):
        ...
        Y la otra que se pasa por parametro el node, busca si tiene
        simbolos asociados entonces aument el numero de simbolos de ese nodo,
        o en el caso que no tenga simbolo asociado devuelve -1
        Parametros:
        node -- nodo a buscar'''
        if len(arg) == 0:
        ...
        # Despues incrementamos en 1
        self.__mSymbolCounter = self.__mSymbolCounter + 1
        self.__mNodeToIdx[nodeid] = newSym
        return newSym

    def AddNodeToIdx(self, idx, node):
        ''' Comprueba si el nodo esta siendo usado, devolviendo
        el indice del simbolo, si es distinto del index del simbolo
        es que esta siendo usado por otro o que no esta siendo usado
        por nadie (-1) entonces lo asociamos al indice del simbolo y
    ...
    def DumpNodes(self):
        ...
        for i in range(0, len(self.__mNodeToIdx)):
            if self.__mNodeToIdx[i] > -1:
                out = out + \
                    "(" + str(self.__mNodeToIdx[i]) + ") " + \
                    str(cSymbolReverseLookup[i]) + " "
        return out
    ...
    def LookupNode(self, node):
        ...
        raise BasicException(
            "Invalid node name found in symboltable node lookup")
