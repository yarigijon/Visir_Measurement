...
def find_first_not_of(*arg):
    '''Busca el primero que no es igual a alguno de los caracteres de chars
        ...
        Parametros:
        arg -- vector que puede tomar diferentes tamaños, donde el primer parametro
        es el caracter, el segundo los caracteres a buscar y el tercero opcional el index 
        donde empezar a buscar. Por defecto es 0.
        '''
    if len(arg) == 3:
        string = arg[0]
        chars = arg[1]
...
def find_last_not_of(*arg):
    ...
    if len(arg) == 3:
        ...
        chars = arg[1]
        pos = 0
    # Descuenta inversa desde longitud del array - 1 y siempre que sea mayor de -1 (es decir 0)
...
def find_last_of(*arg):
    '''Busca en la cadena el último carácter que coincida con cualquiera
        de los caracteres especificados en sus argumentos.

        Cuando se especifica el ultimo parametro de arg, la búsqueda sólo incluye
        caracteres en o antes de la posición pos, ignorando cualquier posible
         ocurrencia después de pos.
...
def Token(*arg):
    ...
    Parametros:
    arg -- es un vector de tamaño variable que contiene al menos la cadena
    de entrada, el numero de tokens y opcionalmente una variable
    booleana que indican los separadores, por defecto tabulador y espacio en blanco.
    '''
    if len(arg) == 3:
        instring = arg[0]
        tokennum = arg[1]
        ...
        tokenchars = "\t "
    pos = 0
    for i in range(0,tokennum+1): # Esto range hace <tokennum por eso se suma 1
...
def RemoveToken(*arg):
    ...
    Parametros:
    arg -- vector de tamaño variable que contiene la cadena de entrada el numero 
    de tokens y como opcional el separador, por defecto, el tabulador y el espacio.
    '''
    if len(arg) == 3:
        instring = arg[0]
        tokennum = arg[1]
...
# TODO: Hay un bug, si pones dos espacios en blanco o espacio en blanco y tabulador
# tal y como esta hecha esta función te daria el espacio en blanco/tabulador de más como token
def Tokenize(*arg):
    '''
...
def GetLine(instring, outstring):
    '''
    Obtiene la primera linea de instring y la devuelve en outstring, además
    elimina de instring esa linea.

    Parametros:
    instring -- cadena de entrada
    outstring -- cadena de salida que contiene la primera linea que encuentra en la 
    cadena de entrada.
    '''
    if instring =="":
        return  instring, "", False # revisar si realmente hay que retornar instring
...
def ToCRLF(instring):
    '''Añade \r a cada salto de linea. y Devuelve el 
    string modificado.
...
def AddWhitespace(instring, length):
    '''Añade tantos espacions en blanco a la cadena como indica length
    
    parametros:
    instring -- cadena a rellenar con espacion en blanco
    lenght -- numero de espacios en blanco a añadir, en realidad
    es lenght menos el tamaño de la cadena lo que se rellenara de espacios en blanco.
    '''
    outstring = instring
