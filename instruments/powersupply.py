...
class PowerSupply(object):
    ...
    def SetPowerSupplyEnable(self, PowerSupplyEnable):
        if not PowerSupplyEnable:
            self.PowerSupplyEnable = 0
        elif PowerSupplyEnable:
            self.PowerSupplyEnable = 1
        else:
            self.PowerSupplyEnable = -1
    ...
    def SetOutPlus25VEnable(self, OutPlus25VEnable):
        if not OutPlus25VEnable:
            self.OutPlus25VEnable = 0
        elif OutPlus25VEnable:
    ...
    def SetOutPlus25VCurrentLimit(self, OutPlus25VCurrentLimit):
        if float(OutPlus25VCurrentLimit) <= 1 or float(
                OutPlus25VCurrentLimit) >= 0:
    ...
    def SetOutMinus25VEnable(self, OutMinus25VEnable):
        if not OutMinus25VEnable:
            self.OutMinus25VEnable = 0
        elif OutMinus25VEnable:
    ...
    def SetOutMinus25VCurrentLimit(self, OutMinus25VCurrentLimit):
        if float(OutMinus25VCurrentLimit) <= 1 or float(
                OutMinus25VCurrentLimit) >= 0:
    ...
    def CheckValues(self):
        return (
            self.Function != -
            1 and self.PowerSupplyEnable != -
            1 and self.Out6VEnable != -
            1 and self.Out6VVoltage != -
            1 and self.Out6VCurrentLimit != -
            1 and self.OutPlus25VEnable != -
            1 and self.OutPlus25VVoltage != -
            1 and self.OutPlus25VCurrentLimit != -
            1 and self.OutMinus25VEnable != -
            1 and self.OutMinus25VVoltage != -
            1 and self.OutMinus25VCurrentLimit != 1)

    def GetEQCommand(self):
        if self.CheckValues():
            if self.Function == 0:
                Command = str(self.InstrumentID) + "\t0 " + \
                    str(self.PowerSupplyEnable) + \
                    " " + str(self.Out6VEnable) + " " + \
                    str(self.Out6VVoltage) + " "
                Command += str(self.Out6VCurrentLimit) + " " + \
                    str(self.OutPlus25VEnable) + " " + \
                    str(self.OutPlus25VVoltage) + " "
                Command += str(self.OutPlus25VCurrentLimit) + " " + str(
                    self.OutMinus25VEnable) + " " + \
                    str(self.OutMinus25VVoltage) + " "
    ...
    def GetXMLResponse(self):
        if self.CheckValues():
            ...
            XML += '<dc_voltage_actual value="' + \
                str(ScientificValue) + '"/>\n'
            ScientificValue = '%.6E' % self.Out6VCurrentMeasurement
            XML += '<dc_current_actual value="' + \
                str(ScientificValue) + '"/>\n'
            XML += '<dc_output_enabled value="1"/>\n'
            XML += '<dc_output_limited value="' + \
                str(self.Out6VCurrentLimitFlat) + '"/>\n' + '</dc_output>\n'
            ...
            XML += '<dc_output_enabled value="1"/>\n'
            XML += '<dc_output_limited value="' + \
                str(self.OutPlus25VCurrentLimitFlat) + \
                '"/>\n' + '</dc_output>\n'
            ...
            XML += '<dc_output_enabled value="1"/>\n'
            XML += '<dc_output_limited value="' + \
                str(self.OutMinus25VCurrentLimitFlat) + '"/>\n' + \
                '</dc_output>\n'
            XML += '</dc_outputs>\n</dcpower>\n'
            return XML
