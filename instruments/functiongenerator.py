...
class FunctionGenerator(object):
	...
	def SetAmplitude(self, Amplitude):
		#La aplicacion web envia la amplitud en Vp y el equipment server la necesita en Vpp por lo que se multiplica por dos mas adelante
	...
	#Como la aplicacion web envia la amplitud en Vp y no Vpp en los archivos .max se utiliza la Vp
		return self.Amplitude
		
	def GetAmplitudeEQ(self):
	#La aplicacion web envia la amplitud en Vp y el equipment server la necesita en Vpp por lo que se multiplica por dos
		return self.Amplitude * 2
		
	def SetFrequency(self, Frequency):
		ValueFrequency = float(Frequency)
		if self.Waveform == 0 or self.Waveform == 1: # senoidal y cuadrada pueden llegar a 20Mhz el resto unicamente a 1Mhz
			if ValueFrequency <= 20000000:
				self.Frequency = ValueFrequency
			else:
				self.Frequency = -1
		elif self.Waveform == 2 or self.Waveform == 3 or self.Waveform == 4 or self.Waveform == 5 or self.Waveform == 6 or self.Waveform == 7:
	...
	def SetDCOffSet(self, DCOffSet):
		...
		if (self.Amplitude * 2 + ValueDCOffSet) <= 10:
			if ValueDCOffSet >= -5 and ValueDCOffSet <= 5:
				...
				self.DCOffSet = -10 # como -1 esta dentro del rango valido utilizamos -10 para indicar error
		else:
				self.DCOffSet = -10	# como -1 esta dentro del rango valido utilizamos -10 para indicar error
	...
	def SetStartPhase(self, StartPhase):
		...
		if ValueStartPhase >= -180 and ValueStartPhase <= 180:
			...
			self.StartPhase = -1000 # como -1 esta dentro del rango vaido utilizamos -1000 para indicar error
	...
	def SetUserDefinedWaveform(self, UserDefinedWaveform):
		self.UserDefinedWaveform = 0 # de momento no implementamos esta funcion, 0 por defecto
		
	def GetUserDefinedWaveform(self):
		return self.UserDefinedWaveform
		
	def CheckValues(self):
		return (self.Function != -1 and self.Waveform != -1 and self.Amplitude != -1 and self.Frequency != -1 
				and self.DCOffSet != -10 and self.StartPhase != -1000 and self.TriggerMode != -1 and self.TriggerSource != -1
				and self.BurstCount != -1 and self.DutyCycleHigh != -1 and self.UserDefinedWaveform != -1)
		
	def GetEQCommand(self):
		if self.CheckValues():
			if self.Function == 0:
				Command = str(self.InstrumentID) + "\t0" + " " + str(self.Waveform) + " " + str(self.Amplitude) + " " + str(self.Frequency) + " "
				Command += str(self.DCOffSet) + " " + str(self.StartPhase) + " " + str(self.TriggerMode) + " " + str(self.TriggerSource) + " "
				Command += str(self.BurstCount) + " " + str(self.DutyCycleHigh) + " " + str(self.UserDefinedWaveform) + "\n"
			else:
	...
	def GetXMLResponse(self):
		if self.CheckValues():
			Waveform = ["sine", "square", "triangle", "rampup", "rampdown", "DC", "noise", "arb"]
			...
			XML += '<fg_triggermode value="' + TriggerMode[self.TriggerMode] + '"/>\n'
			XML += '<fg_triggersource value="' + TriggerSource[self.TriggerSource] +'"/>\n'
